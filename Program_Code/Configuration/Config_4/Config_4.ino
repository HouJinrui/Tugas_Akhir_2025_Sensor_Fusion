...
// Enhanced response time calculation variables (based on reference code)
...
void resetAngles() {
  ...
  // Reset all tracking variables (based on reference code)
  prevFusedAngle = 0.0;
...
void loop() {
  ...
  // ---------- Enhanced Response Time Calculation (Based on Reference Code) ----------
  ...
  switch (currentState) {
    case IDLE:
      if (angleChange >= ANGLE_THRESHOLD) {
        // Movement detected - start tracking
        currentState = MOVING;
        ...
        Serial.print(">>> PERGERAKAN DIMULAI dari: ");
    ...
    case MOVING:
      {
        ...
        if (currentDeviation > 180) currentDeviation = 360 - currentDeviation; // Handle wrap-around
        if (currentDeviation > maxMovementDeviation) {
          maxMovementDeviation = currentDeviation;
        }
        
        // Only process timing if we've passed the movement threshold
        if (maxMovementDeviation >= MOVEMENT_THRESHOLD) {
          ...
          // Still tracking movement, check if it stops before reaching threshold
          if (angleChange < STABILITY_THRESHOLD) {
            stableCount++;
            if (stableCount >= STABLE_COUNT_REQUIRED) {
              // Movement stopped before reaching threshold
              Serial.print(">>> Pergerakan berhenti sebelum 15° (");
              Serial.print(maxMovementDeviation, 1);
              Serial.println("°) - Reset ke IDLE");
              currentState = IDLE;
              stableCount = 0;
            }
          } else {
